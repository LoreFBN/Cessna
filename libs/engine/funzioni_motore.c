...
#include <math.h>
#include <unistd.h>
#include "libreria_motore.h"

void lettura_file_motore(char *nome_file){
    printf("****************************LETTURA DEL FILE MOTORE*******************************\n");
...
//        else if(strcmp(nome_file,"propeller.txt") == 0 || strcmp(nome_file,"dba.txt") == 0 ){
...
void scrittura_file_motore(double *datiengine, char nome_file[20]){
    ...
    while (fgets(stringa,(max_lunghezza_riga),file_di_testo) != NULL){
        if(stringa[0] != '*' && stringa[0] != '\n' ){
                sscanf(stringa,"%lf", &datiengine[indice]);
                printf("posizione: %d -> valore: %.10lf\n",indice,datiengine[indice]);
                indice++;
        }
    }
    fclose(file_di_testo);
